...
### FUNCTIONS
# resize the images, output to /shirts, keeping original file names
...
def predict_embeddings(image_file, model):
    ...
    shirt_info = {
        ...
        'image': image_file,  # Include the 'image' key with the file path
    }
...
def get_random_similar_shirts(all_shirts):
    results = []
    for _ in range(5):
...
def get_shirt_info(images_folder):
    all_shirts = []
...
all_shirts = get_shirt_info(images_folder)
...
def show_results():
    # test the embedings with cosine scoring
    results = get_random_similar_shirts(all_shirts)
    return render_template('test_embeddings.html', results=results)

@app.route('/start_shopping')
def start_shopping():
